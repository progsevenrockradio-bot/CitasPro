...
- **Scroll Automático en Formularios por Pasos:** Todo formulario que se vaya descubriendo por pasos (como la reserva pública o cuestionarios dinámicos) debe incluir auto-scroll hacia la nueva sección recién renderizada usando refs de Vue y `scrollIntoView({ behavior: 'smooth', block: 'start' })`, para que el usuario no se pierda al abrirse el nuevo bloque.
- **Prellenado Inteligente de Datos (No pedir 2 veces lo mismo):** Cuando un formulario multi-paso o un formulario anidado (como la Historia Clínica) solicite datos que el usuario ya ingresó en un paso anterior (ej. Nombre, Apellido, Teléfono, Email), esos campos deben ser **pre-llenados automáticamente** en el estado o en el modelo (`v-model`) para mejorar la UX. Nunca forzar al usuario a escribir su nombre o contacto dos veces en el mismo flujo.
